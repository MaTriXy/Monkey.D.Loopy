 ...
 * crash-resumable Node project that depends only on @loopy/runtime and exposes
 * a human CLI surface (run | step | resume | doctor) plus an agent surface (SKILL.md).
 *
 * The runtime (@loopy/runtime, built in M1) owns the journal/replay/caps machinery;
 ...
 * `@loopy/runtime` package; in `vendor` mode, a local single-file bundle the CLI emits alongside.
 */
function runtimeSpecifier(vendor: boolean): string {
  return vendor ? "./runtime.bundle.mjs" : "@loopy/runtime";
}
...
function emitLoopFile(spec: LoopSpec, vendor = false): string {
  ...
  parts.push("// Generated by Monkey D Loopy (@loopy/core). Do not edit by hand — edit the LoopSpec and recompile.");
...
function emitPackageJson(spec: LoopSpec, vendor = false): string {
  const pkg = {
    ...
    dependencies: vendor ? {} : { "@loopy/runtime": "^0.1.0" },
  };
