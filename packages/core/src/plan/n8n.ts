  ...
  plan(spec: LoopSpec): PlanResult {
    const files: PlannedFile[] = [
      ...
      { relativePath: "README.md", contents: emitReadme(spec), kind: "doc" },
      {
        ...
        contents:
          JSON.stringify({ loop_id: spec.id, loopspec_version: spec.loopspec, target: "n8n", recipe: spec.provenance?.recipe, signal: spec.terminate.signal, caps: spec.caps, artifacts: spec.artifacts, notify: spec.notify }, null, 2) + "\n",
        kind: "provenance",
      },
    ];
